...
- [x] **R1 · Add missing DB indexes** 🔴 S — `database/migrations/`
...
- [x] **R2 · `Work::scopePresent()` / `scopeMissing()`** 🟠 M — `app/Models/Work.php` + 6 files / 18 sites
...
- [x] **R3 · Optimise `WorkSearch::facets()` query shape** 🔴 M — `app/Browse/WorkSearch.php:103-139,66-100`
...
- [x] **R4 · Centralise tag canonicalisation (+ make multi-hop safe)** 🔴 S — `app/Models/Tag.php`, `WorkTagSync.php:61-66`, `WorkTagController.php:25-26`
...
- [x] **R5 · Fix Vite font weights (drop forbidden 500)** 🔴 S — `vite.config.js:11-15`
...
- [x] **R6 · Global `:focus-visible` ring** 🔴 S — `resources/css/app.css`
...
- [x] **R7 · Extract shared CSRF `postJson` helper** 🔴 M — `resources/js/app.js` + 6 Blade components
...
- [x] **R8 · Transaction boundaries for multi-write ops** 🟠 M — `SeriesDetector::detect`, `LibraryScanner` missing-sweep/prune, `SeriesManagementController` (4 ops)
...
- [x] **R9 · Memoise tag canonical-id within a scan** 🟠 M — `app/Tagging/WorkTagSync.php` (driven by scan loop)
...
- [x] **R10 · Dedupe zip open/read; `CoverGenerator` reuses `ZipPageReader`** 🟠 S — `app/Archive/`
...
- [x] **R11 · `Work` card-relations constant/scope** ⚪ S — controllers
...
- [x] **R12 · Single source for the 6 facet dimensions** 🟠 M — `WorkSearch.php:20`, `browse/index.blade.php`
...
- [x] **R13 · Surface corrupt-archive errors in `PageController`** 🟠 S — `app/Http/Controllers/PageController.php:43-45`
...
- [x] **R14 · Run container processes as non-root** 🔴 S — `docker/s6/s6-rc.d/{web,worker,scheduler}/run`
...
- [x] **R15 · Drop weak default DB passwords in compose** 🟠 S — `docker-compose.yml:15,29,30`
...
- [x] **R16 · Remove redundant `/up` health route** 🟠 S — `bootstrap/app.php:12`
...
- [x] **R33 · CI dependency caching** 🟠 S — `.github/workflows/ci.yml`
...
- [x] **R17 · Remove dead `language` field from `ParsedName`** 🟠 S — `app/Parsing/ParsedName.php:20,37,48`
...
- [x] **R18 · Drop dead `series.cover_work_id` column** 🟠 S — `database/migrations/2026_06_21_000002`
...
- [x] **R19 · Make `Tag::AUTO_TYPES` load-bearing** ⚪ S — `app/Models/Tag.php:24`, `WorkTagSync.php:45,53`, `LegacyScalarBackfill.php:18`
...
- [x] **R20 · Move `deriveSortTitle` out of `ParsedName`** ⚪ M — 6 callers (Series/Tag/Tagging/2× Http/Parsing)
...
- [x] **R21 · Extract `Series::pruneEmptyAuto($mangakaId)`** ⚪ S — `SeriesDetector.php:60-63`, `SeriesManagementController.php:110-116`
  "Delete empty auto series" query written verbatim twice. *Action:* one scope/static, call from both.

- [x] **R22 · `Tag` `$casts` for `merged_into_id`** ⚪ S — `app/Models/Tag.php`
...
- [x] **R23 · `Scan` model constants + `scopeActive()`** ⚪ S — `app/Models/Scan.php`, `MaintenanceController.php:37`
...
- [x] **R24 · Fix `composer test` script** ⚪ S — `composer.json:51`
...
- [x] **R25 · Remove `inspire` demo command** ⚪ S — `routes/console.php:8-10`
  Default Laravel scaffold in a single-purpose app. *Action:* delete it + the unused import.

- [x] **R26 · Single source for image extensions** ⚪ S — `ArchiveInspector.php:13`, `config/scan.php:13`, `PageController.php:15-22`
...
- [x] **R27 · Extract shared form/card Blade partials** 🟠 M — `resources/views/` — **PARTIAL**: shipped `x-card-grid`, `x-collection-card`, `x-pagination` (output-identical). Deferred `x-input` / reusing `x-button` for inline buttons / control-padding token migration — those change rendered sizes and want a human design eyeball.
...
- [x] **R28 · A11y: `aria-current` nav + page `<h1>`** 🟠 S — `components/nav.blade.php:7-11`, `home/mangaka-index/maintenance`
...
- [ ] **R31 · Dependency hygiene** ⚪ S — `composer.json`, `package.json` — **DEFERRED**: `laravel/pao` is a legitimate direct dev dep (not unused); moving Pest to `require-dev` forces a full lock re-resolve (risks version bumps) for a purely cosmetic gain (image already `--no-dev`). Not worth the lock churn.
...
- [x] **R32 · Remove empty scaffold stubs / document DI** ⚪ S — `Controller.php`, `AppServiceProvider.php`
...
**Done: 31 of 33** (R1–R30, R32, R33). **Deferred: R31** (rationale above). **Partial: R27** (3 of 6
extractions shipped; visual ones deferred with rationale above).

**Verification (all green):**
- Unit/feature suite: **170 tests, 580 assertions** (was 168/579 at start; +2 new behavioural tests:
  multi-hop alias resolution, two-dimension facet narrowing).
- **Coverage: 100.0%** of `app/` (PCOV) — the baseline held.
- **Pest browser suite: 21 tests** green (real Chromium, light + dark, no console/JS errors) — re-run
  after every interactive-frontend change (R5/R6/R7/R12/R27/R28/R29).
- `npm run build` clean; `php artisan migrate:fresh` clean on SQLite.

**Highlights:** indexed the per-scan `relative_path` lookup + `tags.merged_into_id`; facet counts now
use an id subquery instead of materialising the corpus into PHP; tag canonicalisation centralised &
multi-hop-safe; transaction boundaries on scan/detect/series ops; `Work` present/missing scopes and
`CARD_RELATIONS` kill ~18 duplicated predicates; shared `window.wyd` fetch helper; design-token fixes
(font weights, focus ring, dark error colour); container drops to non-root.

**Needs a human follow-up before relying on it:**
- **R14 (non-root container)** — implemented per the s6/FrankenPHP non-root pattern but **could not be
  booted here**. Run `docker build` + `docker compose up` and confirm covers write to `/data` and the
  web process runs as `www-data`.
- **R27 remainder** + **R31** — see the per-item notes above.
